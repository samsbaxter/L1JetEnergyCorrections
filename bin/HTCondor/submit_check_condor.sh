...
do
    ...
    do
        ...
        echo "$jobname"
        echo "$outname"
        echo "arguments = python checkCalibration.py ${fdir}/${fname} ${fdir}/${outname} --etaInd ${i}" >> "$outfile"
        echo "queue" >> "$outfile"
    done
    outname=${fname#pairs_}
    outname=${outname%.root}
    echo "arguments = python checkCalibration.py ${fdir}/${fname} ${fdir}/check_${outname}_central.root --central ${i}" >> "$outfile"
    echo "queue" >> "$outfile"
    echo "arguments = python checkCalibration.py ${fdir}/${fname} ${fdir}/check_${outname}_forward.root --forward ${i}" >> "$outfile"
    echo "queue" >> "$outfile"
    echo "arguments = python checkCalibration.py ${fdir}/${fname} ${fdir}/check_${outname}_all.root ${i}" >> "$outfile"
    echo "queue" >> "$outfile"
