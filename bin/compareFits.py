...
def compare():
    ...
    for i, (eta_min, eta_max) in enumerate(zip(binning.eta_bins[:-1], binning.eta_bins[1:])):
        ...
        buggy_graphs = [
            # Contribution(file_name=f_0PU_buggy, obj_name="l1corr_eta_%g_%g" % (eta_min, eta_max),
            #              label="0PU (760pre7)", line_style=2, line_color=colors[0], marker_color=colors[0]),
            Contribution(file_name=f_PU0to10_buggy, obj_name="l1corr_eta_%g_%g" % (eta_min, eta_max),
                         label="PU: 0 - 10 (buggy)", line_style=2, line_color=colors[1], marker_color=colors[1]),
            Contribution(file_name=f_PU15to25_buggy, obj_name="l1corr_eta_%g_%g" % (eta_min, eta_max),
                         label="PU: 15 - 25 (buggy)", line_style=2, line_color=colors[2], marker_color=colors[2]),
            Contribution(file_name=f_PU30to40_buggy, obj_name="l1corr_eta_%g_%g" % (eta_min, eta_max),
                         label="PU: 30 - 40 (buggy)", line_style=2, line_color=colors[3], marker_color=colors[3])
        ]
        ylim = None
        if eta_min > 2:
            ylim = [0, 3.5]
        ...
        for i, pu_label in enumerate(['PU0to10', 'PU15to25', 'PU30to40']):
            p = Plot(contributions=[new_graphs[i], buggy_graphs[i]], what="graph", xtitle="<p_{T}^{L1}>", ytitle="Correction value (= 1/response)",
                     title="Spring15 MC, no JEC, Stage 2, %g < |#eta| < %g" % (eta_min, eta_max), ylim=ylim)
            p.plot()
            p.save(os.path.join(oDir, "compare_800pre5_buggyPUS_eta_%g_%g_%s.pdf" % (eta_min, eta_max, pu_label)))
