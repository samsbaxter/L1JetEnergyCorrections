...
L1ExtraParticlesProd::L1ExtraParticlesProd(const edm::ParameterSet& iConfig)
   : produceMuonParticles_( iConfig.getParameter< bool >(
	...
	"tauJetSource" ) ),
     gctInternJetSource_( iConfig.getParameter< edm::InputTag >(
	"gctInternJetSource" ) ),
     genJetSource_( iConfig.getParameter< edm::InputTag >(
	"genJetSource" ) ),
     ...
     ignoreHtMiss_( iConfig.getParameter< bool >( "ignoreHtMiss" ) )
{
   ...
   consumes<L1GctInternJetDataCollection>(gctInternJetSource_);
   // gctInternJetToken_ = consumes<L1GctInternJetDataCollection>(gctInternJetSource_);
   consumes<reco::GenJetCollection>(genJetSource_);
		...
		const edm::EventSetup& iSetup)
{
   ...
   auto_ptr< L1JetParticleCollection > tauJetColl(
	...
	auto_ptr< L1JetParticleCollection > gctInternJetColl(
			new L1JetParticleCollection );

	auto_ptr< L1JetParticleCollection > genJetColl(
			new L1JetParticleCollection );
	...
	if( produceCaloParticles_ )
	{
			...
			for( int i = 0 ; jetItr != jetEnd ; ++jetItr, ++i )
			{
						...
						( !centralBxOnly_ || jetItr->bx() == 0 ) )
				{
					...
					cenJetColl->push_back(
							L1JetParticle( gctLorentzVector( et, *jetItr, caloGeom, true ), Ref< L1GctJetCandCollection >( hwCenJetCands,i ), jetItr->bx() ) ) ;
				}
			}
		}
		...
		// GCT Internal jets
		Handle<L1GctInternJetDataCollection> hwGctInternJetCands;
		// iEvent.getByToken(gctInternJetToken_, hwGctInternJetCands);
		iEvent.getByLabel(gctInternJetSource_, hwGctInternJetCands);

		if( !hwGctInternJetCands.isValid() )
		{
			LogDebug("L1ExtraParticlesProd")
			<< "\nWarning: L1GctInternJetDataCollection with " << gctInternJetSource_
			<< "\nrequested in configuration, but not found in the event."
			<< std::endl;
		}
		else
		{
			L1GctInternJetDataCollection::const_iterator gctItr = hwGctInternJetCands->begin();
			L1GctInternJetDataCollection::const_iterator gctEnd = hwGctInternJetCands->end();
			for( ; gctItr != gctEnd ; ++gctItr) {
				if (!gctItr->empty() && (gctItr->et() != 0)){
					cout << "Jet: " << gctItr->bx() << " : " << gctItr->rank() << " : " << gctItr->et() << " : "  << gctItr->eta() << " : " << gctItr->sgnEta() << " : " << gctItr->phi() << endl;
					// double et = gctItr->rank() * (*jetScale).linearLsb();
					double et = gctItr->et() * jetScale->linearLsb();
					double eta = caloGeom->globalEtaBinCenter(gctItr->eta());
					double phi = caloGeom->emJetPhiBinCenter(gctItr->phi());
					double mass = 0.;
					std::cout << et << " (" << gctItr->et() << ") : "  << eta << " : " << phi << std::endl;

					gctInternJetColl->push_back(
						L1JetParticle(math::PtEtaPhiMLorentzVector(et, eta, phi, mass),
		...
		// Gen jets
		Handle<reco::GenJetCollection> hwGenJetCands;
		// iEvent.getByToken(gctInternJetToken_, hwGenJetCands);
		iEvent.getByLabel(genJetSource_, hwGenJetCands);

		if( !hwGenJetCands.isValid() )
		{
			LogDebug("L1ExtraParticlesProd")
			<< "\nWarning: reco::GenJetCollection with " << genJetSource_
			<< "\nrequested in configuration, but not found in the event."
			<< std::endl;
		}
		else
		{
			reco::GenJetCollection::const_iterator genItr = hwGenJetCands->begin();
			reco::GenJetCollection::const_iterator genEnd = hwGenJetCands->end();
			for( ; genItr != genEnd ; ++genItr) {
				// if (!genItr->empty()){
					genJetColl->push_back(
						L1JetParticle(genItr->p4(),
										L1JetParticle::JetType::kUndefined,
										0)
						);
				// }
   ...
   OrphanHandle< L1JetParticleCollection > tauJetHandle =
	...
	OrphanHandle< L1JetParticleCollection > gctInternJetHandle =
			iEvent.put( gctInternJetColl, "GctInternalJets" ) ;

	OrphanHandle< L1JetParticleCollection > genJetHandle =
			iEvent.put( genJetColl, "GenJets" ) ;
